...
import CallToAction from "../../Home/CTA/CallToAction";
import Footer from "../../Home/Footer/Footer";
import { Routes, Route, Link } from "react-router-dom";

function Product({ sharedData ,selectedProduct, setSelectedProduct,  activeProduct, setActiveProduct}) {
  ...
  const filterNames = products.filter((product) =>
    product.name?.toLowerCase().includes(sharedData.toLowerCase()),
  );

   const handleSelectProduct = (recipe) => {
    setSelectedProduct(!selectedProduct);
    setActiveProduct(recipe);
  }
    ...
    <>
      ...
      <div className={style.productContainer}>
        {filterNames.length === 0 ? (
          <p className={style.errorMessageForProduct}>No product found</p>
        ) : filterNames.map((recipe) => (
          <div key={recipe.id} className={style.productCard} >
            <div className={style.productImage}>
              <img src={recipe.images} alt="" />
            </div>
            <h4>{recipe.name}</h4>
            <p>{recipe.description}</p>
            <div className={style.serveCookContainer}>
              <span className={style.servingContainer}>
                <svg
                  width="20px"
                  height="20px"
                  viewBox="0 0 24 24"
                ...
                >
                  ...
                  <g id="SVGRepo_iconCarrier">
                    {" "}
                    <circle cx="12" cy="6" r="4" fill="#163A34" />{" "}
                    <path
                ...
                <p className={style.servingText}>Servings {recipe.serving}</p>
              </span>
              <span className={style.dishContainer}>
                <svg
                  fill="#163A34"
                  height="20px"
                  width="20px"
                ...
                <p className={style.cookTime}>
                  Total cooking time: {recipe.cooktime}
                </p>{" "}
              </span>
            </div>

            <Link to="/recipes/details"  state={{ recipe }}> <button className={style.viewRecipeBtn} onClick={() => handleSelectProduct(recipe)}>View recipes</button> </Link>

          </div>
        )) }
      
        {selectedProduct === true ? <PopUp /> : null}
      </div>
      <Footer />
    </>
  ); 
